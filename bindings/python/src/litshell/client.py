...
This module provides the main client class for connecting to lit-shell servers.
"""
...
class XShellClient:
    """WebSocket client for lit-shell terminal server.

    Provides methods to:
    - Connect to lit-shell server
    ...
    async def connect(self) -> ServerInfo:
        """Connect to the lit-shell server.
        ...
        """
        logger.info(f"Connecting to lit-shell at {self.url}")
        ...
        logger.info(
            f"Connected to lit-shell. Docker enabled: {self.server_info.docker_enabled}"
        )
    ...
    async def spawn(
        ...
        """
        if not self.ws:
            raise RuntimeError("Not connected to lit-shell server")
    ...
    async def close(self) -> None:
        """Close the connection to lit-shell server."""
        self._running = False
        ...
        logger.info("Disconnected from lit-shell")
...
class XShellClientSync:
    ...
    def __init__(self, url: str):
        ...
        Args:
            url: WebSocket URL for lit-shell server
        """
        self.url = url
